"""
Test suite for Project B (Post-Change) using pytest
...
def service():
    """Create cart service instance with adapter"""
    return CartServiceV2(
...
def results_collector():
    """Collect test results and statistics"""
    return {
        'results': [],
...
def test_api_v2(service, test_data, results_collector, test_case_id):
...
def test_save_results(service, results_collector):
    ...
    # Get adapter stats
    adapter_stats = service.adapter.get_stats() if service.use_adapter else {}
    ...
    summary = {
        ...
        "adapter_stats": {
            "fallback_count": adapter_stats.get('fallback_count', results_collector['fallback_count']),
            "retry_count": adapter_stats.get('retry_count', 0),
            "circuit_breaker_state": adapter_stats.get('circuit_breaker_state', 'N/A'),
            "async_cases": results_collector['async_cases']
        },
    ...
    print(f"Async Cases: {results_collector['async_cases']}")
