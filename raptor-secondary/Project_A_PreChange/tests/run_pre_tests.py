...
from mocks import mock_server_v1  # type: ignore
from src.cart_service_v1 import check_availability  # type: ignore
...
METRICS_FILE = RESULTS_DIR / "metrics_pre.json"
...
def main():
    # Start mock server (or reuse if already running)
    # Pick a free port for isolation
    def find_free_port():
    ...
    def wait_for_v1(max_wait_sec=5):
        url = f"http://127.0.0.1:{v1_port}/api/v1/checkStock"
        start = time.time()
    ...
    console.log(f"Ensuring V1 mock server on {v1_port}")
    started = False
    if not wait_for_v1(max_wait_sec=1):
        mock_server_v1.start_in_thread(port=v1_port)
        started = True
        time.sleep(1.0)
        if not wait_for_v1(max_wait_sec=5):
    ...
    for case in test_cases:
        ...
        result = check_availability(
            ...
            base_url=f"http://127.0.0.1:{v1_port}",
        )
        ...
        passed = (
            ...
            and result.get("fallback_used") == expected.get("fallback_used")
        )
        if passed:
            pass_count += 1
        result_entry = {
    ...
    # Also log to log_pre.txt
    with open(LOG_FILE, "w", encoding="utf-8") as logf:
        for r in results:
            logf.write(json.dumps(r) + "\n")

    console.log(f"Completed Project A tests. Passed {pass_count}/{len(test_cases)}")
